...
验证基线：agent-server 全套 116 测试通过；`npm run check` 干净；live E2E 9 项检查全部通过（见 ` design/2026-07-21-agent-server-p1-live-verification.md`），session 文件经 pi 自己的 `JsonlSessionStorage` 回读验证。
...
- 各任务决策记录：` design/2026-07-21-agent-server-task{6,7,8,9}-*-changes-and-decisions.md`。
