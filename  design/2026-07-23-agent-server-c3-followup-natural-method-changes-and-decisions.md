...
日期：2026-07-23
来源：C3 验收报告（` design/2026-07-23-agent-server-c3-acceptance-report.md`）"遗留与后续建议"第 1 条——构造含 `retry`/`backoff` 关键词的 session 重跑 `runDailyEvolution`，把场景 1 的"条件性 PASS"升级为完整 PASS。
通用约束：见 ` design/2026-07-22-agent-server-p3-candidate-tasks.md`"通用约束"一节。本次为数据构造 + live 验证 + 文档刷新，**零代码改动**。
...
| ` design/2026-07-23-agent-server-c3-live-verification.md` | 场景 1 判定：条件性 PASS → PASS（补 follow-up 小节） |
| ` design/2026-07-23-agent-server-c3-observation-baseline.md` | 全量刷新：库存/quality 分布/checkpoint/会话特征/迭代建议（含 FTS 新发现第 5 条） |
| ` design/2026-07-23-agent-server-c3-followup-natural-method-changes-and-decisions.md` | 本文档 |
| ` design/INDEX.md` | 新增本文档条目；C3 相关条目措辞同步；时间线补 follow-up 记录 |

测试基线：零代码改动，未动 213 测试；`npm run check` 由 pre-commit 执行。

Refer Spec：` design/2026-07-22-agent-server-c-ability-distillation-design-and-tasks.md`（C3 场景 1）；` design/2026-07-23-agent-server-c3-acceptance-report.md`（遗留建议第 1 条）；` design/2026-07-23-agent-server-c3-live-verification.md`；` design/2026-07-23-agent-server-c3-observation-baseline.md`
