...
- **omlx 不可动**：禁止为了通过测试或验证而修改 omlx 的任何配置，包括当前已配置好的模型（`~/.omlx/settings.json`、模型目录 `/Volumes/extern-1t-x5/models` 及其内容、运行中实例的端口/密钥/模型加载状态）。omlx 只可按现状使用（启动/查询/调用）；环境不满足时报告阻塞，不得改动环境迁就测试。
