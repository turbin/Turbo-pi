...
日期：2026-07-22
来源：`doc/design/2026-07-22-agent-server-p2-closeout.md` 的 P3 候选清单。
...
- 每个任务 = 1 个提交（实现 + 测试 + 决策记录文档），单提交 ≤ 3000 行。决策记录落 `doc/design/<date>-agent-server-p3-task<N>-*-changes-and-decisions.md`（注意目录名带前导空格）。
- 提交信息格式（用户约定，不得更改）：
  ```text
  COMPLETED：<描述完成的任务>
  TODO：<描述待完成的任务>
...
- **改动范围仅限当前工程**：只允许修改本仓库（pi-monorepo 工作目录）内的文件——源码、测试、文档、`var/` 等 runtime 产物、工程内配置文件（如 `packages/agent-gateway/config.toml`）。工程之外的任何东西一律不得改动，包括但不限于：
  ...
  - 不允许为了通过测试或验证而改动环境迁就测试。任务若需要工程外的配合（改某个配置、装某个东西、起某个服务），**停下来报告用户**，由用户处理，不得自行越权操作。
  - 临时文件（/tmp 下的一次性脚本、smoke 输入）可以创建，用后删除；这不视为"改动"。
- **跑批前置核验（2026-08-19 用户新增）**：启动任何跑批/测试前，必须按 `doc/design/2026-08-19-run-batch-preflight-checklist.md` 逐项核验测试环境与条件全部就绪（模型层/gateway/agent-server/judge/Langfuse/跑批进程/判据门七类）；探针类项目必须看真实响应或真实数据，"进程在"不算证据；任何一项不过，停下来报告用户，不得开跑。
...
5. 产出验证文档 `doc/design/<date>-agent-server-p3-real-llm-verification.md`。
