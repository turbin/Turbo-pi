...
- live E2E 8 项检查全部 PASS（见 `doc/design/2026-07-22-agent-server-p2-live-verification.md`），流式 session 经 pi `JsonlSessionStorage` 回读验证，离线 evolution 全链路（ETL→pipeline→promote→rescore→cleanup→checkpoint）真实跑通。
