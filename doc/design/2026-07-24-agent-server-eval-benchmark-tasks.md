...
**通用约束**：见 `doc/design/2026-07-22-agent-server-p3-candidate-tasks.md`"通用约束"。新增：第三方 benchmark 工具（mini-swe-agent、terminal-bench、swebench）**必须装在仓库内隔离环境**（`packages/agent-server/eval/.venv`，gitignored），不进系统 Python、不动 omlx。
...
2. 对照成功判据三条出总报告 `doc/design/<date>-agent-server-eval-report.md`：A/B delta、轮间 delta、成本曲线、失败分类、结论（成功/部分成功/失败 + 依据）。
