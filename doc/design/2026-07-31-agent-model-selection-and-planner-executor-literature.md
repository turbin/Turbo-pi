...
## 4.7 追加：skill 库对规划能力的提升与边界（2026-08-04，多源检索）

针对"skill 能否提升 agent 规划能力"，核心证据（SoK: Agentic Skills 综述 2602.20867 + SkillsBench + ASI 等）：

| 发现 | 数据 | 对本项目的意义 |
|---|---|---|
| 精选 skill 提升规划 | 平均 +16.2pp；聚焦型（2-3 模块）+18.6pp | 正 |
| **自生成 skill 无验证则有害** | 平均 **-1.3pp**（5 配置仅 1 正） | 验证 A-D4「入库前回放验证」是必需而非可选 |
| **skill 是算力均衡器** | Haiku 4.5+skills 27.7% > Opus 4.5 裸跑 22.0% | 「小模型+技能库 > 大模型裸跑」已实证——学生-老师路线同向 |
| 百科全书式 skill 降分 | -2.9pp | 粒度纪律（Method/Guard ≤5 上限合理） |
| 已擅长任务上 skill 有害 | 16/84 负收益（最差 -39.3pp） | 注入需适用性判断（命中质量 > 命中率） |
| ASI（2504.06821） | WebArena +23.5% / 步数 -10.7-15.3%；程序化验证是收益主因；代码 skill 宜动作空间、文本宜记忆 | 与 AWM/Skill-DISCO 互证 |
| Skill 粒度随骨干变化（2605.30723，ALFWorld） | 最优粒度逐模型不同 | 与 CoT 蒸馏 ZPD 原则互证；换型后粒度需重校 |
| 预算约束研究（2606.15017） | skill/记忆 token 开销不总值得 | 注入成本入判据③ |

结论：skill 能提升规划能力，前提四条件——**验证门控、聚焦粒度、适用性判断、格式匹配用途（代码→动作/文本→记忆）**。本项目 D1（入库验证双门）/D2（方法级进料）/Method-Guard 上限设计均与该证据链一致。
