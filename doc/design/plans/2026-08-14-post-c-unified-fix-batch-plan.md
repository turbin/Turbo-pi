...
- 归因对象与口径（v2 重写，审查 F-2/F-6/F-7/F-14/F-15）：
  ...
  2. 奖惩规则：注入且高分→加分（+0.1 封顶 1.0）；注入且连续失败→**降权**（confidence ×0.5 且封顶 0.3）。**样本单位 = 任务日（(day, task_id)，M3 预注册修订）**——原方案字面"≥3 个不同任务"在重复集场景永不触发（单任务卡 distinct_tasks=1），且与验收判据（回放标出 issue-010 卡）矛盾，M3 实施时修订为任务日口径并在决策记录完整披露；跨日同任务视为独立样本（judge 独立评分）。风险边界：单任务 confound 可误伤有效卡，但首版仅降权（可逆、误伤有界 ≤0.3 标记带）+ 降级需人工确认，可控；`distinct_tasks>=2` 必要条件列为 D campaign 校准调参项。阈值 ≥3 个失败任务日（预注册）。<阈值不动；多卡共注入首版仅记数不动作
