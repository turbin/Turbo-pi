...
| [issue-010](issue-010-card-guided-execution-crowds-out-deliverable.md) | 照卡执行挤占交付本能——注入卡片致重复集分数连续下滑（0.567→0.404），蒸馏模板缺交付物维度+验证闸门盲区 | fixed（2026-08-14 F1/T2 实施完成，待观察；存量卡重蒸待用户确认时机后排期） | 2026-08-12（D3 数据+个案分析） | 2026-08-14（deliverables schema 三处 + 打分侧封顶 0.49 + TS 非空校验双闸 + restill 重蒸脚本交付） | `test/regressions/issue-010-card-deliverable-gate.test.ts`（11 例）+ `python/tests/test_issue010_deliverable_check.py`（12 例）+ `test_issue010_restill.py`（4 例） |
