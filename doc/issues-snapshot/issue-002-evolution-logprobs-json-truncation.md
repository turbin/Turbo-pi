...
- 状态：**fixed（2026-08-09 补回归测试后转正，待观察一个发布周期）**
- 发现：2026-08-06 16:58 首败（r2）；r3/r4 连续暴露后续层（08-06 23:08 / 08-07 02:01）
- 修复：2026-08-06 ~ 08-07 三轮递进修复（见下）；回归测试 2026-08-09 补齐（`python/tests/test_issue002_pipeline_resilience.py`，用户 08-06 决定草案→到期提醒决策，本次落地转正）
...
## 待办（2026-08-09 到期提醒：B 热库轮已定稿，触发时点已到）

1. ✅ 回归测试已补（`python/tests/test_issue002_pipeline_resilience.py`：截断 JSON/缺 choices 重试双副本 + `_score_once` thinking/max_tokens 注入哨兵，32 测试全绿）——本次落地，issue 转正为 fixed
2. 是否立项"管线分阶段断点持久化"（结构性改进，消除全量重跑放大器）——**仍待用户决策**
3. 或降级为已知风险 / 关闭——**仍待用户决策**
